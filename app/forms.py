...
class ResponsibilityWaste(FlaskForm):
    ...
    eighteenth = RadioField('Сдаете в переработку макулатуру?', choices = ('Да, постоянно', 'Иногда', 'Нет'))

    twentyth = RadioField(' Какой объем макулатуры в процентах вы сдаете в переработку? ', choices = ('до 50%', '50%', 'от 50% до 100%'))
    twentyfirst = RadioField(' Сдаете в переработку металл (алюминий, фольгу, металлические предметы и упаковку)? ', choices = ('Да, постоянно', 'Иногда', 'Нет'))
    twentysecond = RadioField(' Какой объем металла в процентах вы сдаете в переработку? ', choices = ('до 50%', '50%', 'от 50% до 100%'))
    twentythird = RadioField(' Сдаете в переработку пластиковую тару? ', choices = ('Да, постоянно', 'Иногда', 'Нет'))
    twentyfourth = RadioField(' Какой объем пластика в процентах вы сдаете в переработку? ', choices = ('до 50%', '50%', 'от 50% до 100%'))
    twentyfifth = RadioField(' Сдаете в переработку стекло? ', choices = ('Да, постоянно', 'Иногда', 'Нет'))
    twentysixth = RadioField(' Какой объем стекла в процентах вы сдаете в переработку? ', choices = ('до 50%', '50%', 'от 50% до 100%'))
    twentyseventh = RadioField(' Сдаете ли в переработку тетрапак и их аналоги? ', choices = ('Да, постоянно', 'Иногда', 'Нет'))
    twentyeighth = RadioField(' Сколько процентов тетрапака и аналогов сдаете в переработку? ', choices = ('до 50%', '50%', 'от 50% до 100%'))
    twentyninth = RadioField(' Сдаете в переработку ртутные лампы? ', choices = ('Да, постоянно', 'Иногда', 'Нет'))
    thirtieth = RadioField(' Сдаете в переработку батарейки? ', choices = ('Да', 'Нет'))
    thirtyfirst = RadioField(' Сдаете в переработку аккумуляторы? ', choices = ('Да, постоянно', 'Иногда', 'Нет'))
    thirtysecond = RadioField(' Участвуете в акции "Добрые крышечки" или в других подобных акциях? ', choices = ('Да', 'Нет'))
    thirtythird = RadioField(' Сдаете в переработку электротехнику и электронику? ', choices = ('Да, постоянно', 'Иногда', 'Нет'))
    thirtyfourth = RadioField(' Сдаете в переработку отслуживший текстиль? ', choices = ('Да, постоянно', 'Иногда', 'Нет'))
    thirtyfifth = RadioField(' Ремонтируете ли вещи, чтобы дать им "вторую жизнь"? ', choices = ('Да, постоянно', 'Иногда', 'Нет'))
    thirtysixth = RadioField(' Пользуетесь ли шеринговыми платформами для сдачи отслуживших вещей и предметов еще пригодных для использования? ', choices = ('Да, постоянно', 'Иногда', 'Нет'))
    thirtyseventh = RadioField(' Перерабатываете ли пищевые отходы? ', choices = ('Да, постоянно', 'Иногда', 'Нет'))
    ...
    fourtysecond = RadioField(' Сколько пищевых отходов в процентах, по вашему мнению, перерабатывают в вашей организации? ', choices = ('до 50%', '50%', 'от 50% до 100%'))
